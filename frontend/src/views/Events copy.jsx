...
const Events = () => {
  ...
  const [isAttendanceTurnOn, setIsAttendanceTurnOn] = useState(false)
  ...
  const handleFileChange = (e) => {
    ...
    if (name === "picture") {
     ...
     const handleAddEventSubmit = async (e) => {
			...
			const confirmAttendance = async (attendance) => {
				setIsAttendanceTurnOn(attendance);
				...
				if (newEvent.documents.length > 0) {
					try {
						const uploadPromises = newEvent.documents.map(
							async (doc, index) => {
...
const AttendanceModal = ({ confirmAttendance, closeModal }) => {
	return (
		<Modal>
			<ModalContent>
				<ModalHeader>
					<ModalTitle>Attendance</ModalTitle>
					<CloseButton onClick={closeModal}>&times;</CloseButton>
				</ModalHeader>
				<ModalBody>
					<p>Do you want to turn on attendance for this event?</p>
					<ButtonContainer>
						<Button onClick={() => confirmAttendance(true)}>Yes</Button>
						<Button onClick={() => confirmAttendance(false)}>No</Button>
					</ButtonContainer>
				</ModalBody>
			</ModalContent>
		</Modal>
