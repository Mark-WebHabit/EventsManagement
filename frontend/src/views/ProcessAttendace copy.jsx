		...
		async function processAttendace() {
			...
			try {
				...
				if (status.toLowerCase() === "ongoing") {
					// Double-check if attendance is already recorded
					const updatedAttendanceSnapshot = await get(attendanceQuery);
					const updatedAttendance = updatedAttendanceSnapshot.val();

					if (updatedAttendance) {
						const hasAttended = Object.values(updatedAttendance).find(
							(attendee) => attendee.userId === user?.uid
						);
						...
						if (hasAttended) {
							setAttended(true);
							setMessage(
								"You have already done your attendance for this event"
							);
							setTimeIn(hasAttended.timeIn);
							return;
						}
